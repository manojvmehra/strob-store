...
export default function StrobStore() {
  ...
  const handleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: 'https://strob-store.vercel.app' // Change this to localhost:5173 for local testing
      }
    });
...
### **Phase 3: Database Permission (Important)**
For the code to save the user's name, we need to create the tables in Supabase.

1.  Go to **Supabase Dashboard** -> **SQL Editor** (Icon on left with `>_`).
2.  Click **"New Query"**.
